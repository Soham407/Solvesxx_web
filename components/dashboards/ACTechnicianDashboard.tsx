...
import { Wrench, Camera, CheckCircle, Clock, MapPin, AlertCircle, Loader2, Navigation, FileText, Wind } from "lucide-react";
...
const AC_PPE_ITEMS = [
  ...
  "Safety goggles",
  "Insulated tools",
  "Refrigerant handling kit",
  "Fall protection harness (for elevated units)",
];

export function ACTechnicianDashboard() {
    ...
    return (
      <div className="max-w-md mx-auto flex flex-col items-center justify-center min-h-[60vh] gap-4 p-6">
        ...
        <p className="text-sm text-muted-foreground text-center">
          {profileError || "Please log in to access the AC Technician Portal."}
        </p>
  ...
  // Find certifications for current technician
  const myTechnicianProfile = technicians.find(
    (t) => t.employee_id === employeeId
  );
  const certifications = myTechnicianProfile?.certifications ?? [];

  return (
    <div className="max-w-md mx-auto space-y-6 pb-20">
      <div className="flex flex-col text-left">
        <h1 className="text-xl font-bold">AC Technician Portal</h1>
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex flex-col">
                    <span className="text-sm font-bold">{job.title || job.service_name || "AC Service Request"}</span>
      ...
      <Card className="border-none shadow-card ring-1 ring-border">
        <CardHeader className="bg-muted/5 border-b pb-3">
          <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">My Certifications</CardTitle>
        </CardHeader>
        <CardContent className="p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {certifications.map((cert, index) => (
                <Badge
                  key={index}
                  variant="outline"
                  className="text-[10px] font-bold uppercase bg-primary/5 text-primary border-primary/20"
                >
                  {cert}
                </Badge>
              ))}
      ...
      <Card className="border-none shadow-card ring-1 ring-border">
        <CardHeader className="bg-muted/5 border-b pb-3">
          <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Daily PPE Checklist</CardTitle>
        </CardHeader>
        <CardContent className="p-4">
          <ul className="space-y-2">
            {AC_PPE_ITEMS.map((item, index) => (
