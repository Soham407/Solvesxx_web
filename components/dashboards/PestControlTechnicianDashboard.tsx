...
import { Wrench, Camera, CheckCircle, Clock, MapPin, AlertCircle, Loader2, Navigation, FileText, FlaskConical } from "lucide-react";
...
const PEST_PPE_ITEMS: PPEItem[] = [
  { label: "Full body protective suit", isCritical: false },
  { label: "Chemical-resistant gloves", isCritical: false },
  { label: "N95 respirator mask", isCritical: false },
  { label: "Safety goggles", isCritical: false },
  { label: "Chemical spill kit (verify availability before starting)", isCritical: true },
  { label: "Emergency eyewash station (confirm location)", isCritical: true },
];

export function PestControlTechnicianDashboard() {
    ...
    return (
      <div className="max-w-md mx-auto flex flex-col items-center justify-center min-h-[60vh] gap-4 p-6">
        ...
        <p className="text-sm text-muted-foreground text-center">
          {profileError || "Please log in to access the Pest Control Technician Portal."}
        </p>
  ...
  return (
    <div className="max-w-md mx-auto space-y-6 pb-20">
      <div className="flex flex-col text-left">
        <h1 className="text-xl font-bold">Pest Control Technician Portal</h1>
      ...
      <Card className="border-none shadow-card ring-1 ring-border">
        <CardHeader className="bg-muted/5 border-b pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Inventory Alerts</CardTitle>
      ...
      {/* Chemical Expiry Warning */}
      <Card className="border-none shadow-card ring-1 ring-warning/30 bg-warning/5">
        <CardContent className="p-4">
          <div className="flex items-center gap-3">
            <AlertCircle className="h-5 w-5 text-warning shrink-0" />
            <div>
              <p className="text-sm font-bold text-warning">Chemical Expiry Monitoring</p>
              <p className="text-xs text-muted-foreground">Check all chemical batch expiry dates. Discard chemicals within 30 days of expiry.</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-none shadow-card ring-1 ring-border">
        <CardHeader className="bg-muted/5 border-b pb-3">
          <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">PPE Mandatory Checklist</CardTitle>
        </CardHeader>
        <CardContent className="p-4">
          <ul className="space-y-2">
            {PEST_PPE_ITEMS.map((item, index) => (
