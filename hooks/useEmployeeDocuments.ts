...
import {
  mapEmployeeDocumentRows,
  type DocumentStatus,
  type DocumentType,
  type EmployeeDocument,
  type EmployeeDocumentRow,
} from "@/src/lib/employee-documents/employeeDocumentTransforms";
...
export { DOCUMENT_STATUS_CONFIG, DOCUMENT_TYPE_LABELS } from "@/src/lib/employee-documents/employeeDocumentTransforms";
...
export function useEmployeeDocuments(initialFilters?: UseEmployeeDocumentsFilters) {
  ...
  const fetchDocuments = useCallback(async (filterOverrides?: UseEmployeeDocumentsFilters) => {
    try {
      ...
      // Transform data
      const documentsWithDetails: EmployeeDocument[] = mapEmployeeDocumentRows((data || []) as EmployeeDocumentRow[]);
  ...
  const getExpiringDocuments = useCallback(async (
    ...
    try {
      ...
      return mapEmployeeDocumentRows((data || []) as EmployeeDocumentRow[]);
    } catch (err: unknown) {
  ...
  const getRequiredDocumentStatus = useCallback(async (
    ...
    try {
      ...
      ((data || []) as EmployeeDocumentRow[]).forEach((doc) => {
