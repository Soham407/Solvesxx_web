...
for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
  ...
  const { issues } = JSON.parse(planMatch[1]);
  ...
  for (const issue of issues) {
    ...
    try {
      execFileSync(
        "gh",
        ["issue", "close", issue.id, "--comment", "Merged via sandcastle"],
        {
          cwd,
          stdio: "pipe",
        },
  ...
  const completedIssues = settled
    .map((result, i) => ({ result, issue: issues[i] }))
    .filter((entry) => entry.result.commits.length > 0)
    .map(({ issue }) => issue);
  ...
  const mergeResult = await sandcastle.run({
    ...
    promptArgs: {
      ...
      CLOSE_COMMANDS: completedIssues
        .map(
          (issue) => `gh issue close ${issue.id} --comment "Merged via sandcastle"`,
        )
        .join("\n"),
    },
  });
