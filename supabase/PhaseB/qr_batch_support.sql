...
-- Create RLS policies
CREATE POLICY "Allow admins to manage batch logs"
    ON qr_batch_logs
    FOR ALL
    TO authenticated
    USING (EXISTS (
...
CREATE POLICY "Allow users to view batch logs"
    ON qr_batch_logs
    FOR SELECT
    TO authenticated
    USING (true);
...
SELECT 
    ...
    COALESCE(e.full_name, 'Unknown') as generated_by_name,
    qb.count as batch_count,
    a.asset_name as linked_asset_name,
    a.asset_tag as linked_asset_tag,
    w.warehouse_name
FROM qr_codes qc
...
COMMENT ON TABLE qr_batch_logs IS 'Logs for bulk QR code generation batches';
COMMENT ON COLUMN qr_codes.batch_id IS 'Reference to the batch this QR code was generated in';
COMMENT ON COLUMN qr_codes.sequence_number IS 'Sequential number within the batch';
COMMENT ON COLUMN qr_codes.is_linked IS 'Whether this QR code is linked to an asset';
COMMENT ON FUNCTION get_unlinked_qr_codes IS 'Returns unlinked QR codes available for asset assignment';
COMMENT ON FUNCTION get_qr_batch_statistics IS 'Returns statistics about QR code batches for a society';
