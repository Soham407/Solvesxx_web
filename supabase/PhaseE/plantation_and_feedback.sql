...
WITH CHECK (true); -- In production, verify resident_id matches auth.uid relation

-- 7. Seed Data
-- Get a society_id and employee_id for seeding
DO $$
DECLARE
    v_society_id UUID;
    v_employee_id UUID;
...
BEGIN
    ...
    IF v_society_id IS NOT NULL THEN
        ...
        -- Mapping services to suppliers for scorecard test
        ...
        IF v_supplier_id IS NULL THEN
            SELECT id INTO v_supplier_id FROM suppliers WHERE supplier_name = 'Green thumb Ventures';
        END IF;
        ...
        -- Feedback
        SELECT id, service_id INTO v_request_id, v_service_id FROM service_requests LIMIT 1;
