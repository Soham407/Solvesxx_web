...
-- Combined Budget Update & Alert Function
...
BEGIN
    IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') AND NEW.budget_id IS NOT NULL THEN
        ...
        -- Check and Notify relevant roles (Admin, Finance, MD)
        SELECT * INTO v_budget FROM budgets WHERE id = NEW.budget_id;
        IF v_budget.allocated_amount > 0 THEN
            ...
            END IF;
        END IF;
    ELSIF (TG_OP = 'UPDATE' OR TG_OP = 'DELETE') AND OLD.budget_id IS NOT NULL THEN
        -- Recalculate old budget if it changed or bill was deleted
        UPDATE budgets b
        SET used_amount = (SELECT COALESCE(SUM(total_amount), 0) FROM purchase_bills WHERE budget_id = b.id),
            updated_at = NOW()
